...
import cz.cvut.fel.integracniportal.domain.user.events.UserEmailVerificationExpiredEvent;
...
public class UserEmailVerificationSaga extends AbstractExpirableUserTokenSaga {
    ...
    public void handle(UserEmailVerificationStartedEvent event) {
        String token = createTokenAndScheduleExpireEvent(
                event.getUserId(),
                new UserEmailVerificationExpiredEvent(event.getUserId()));
...
//        user.getEmail();
    ...
    @SagaEventHandler(keyName = "userId", associationProperty = "userId")
    public void handle(UserEmailVerificationExpiredEvent event) {
        finish(event.getUserId());
    }
